    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        ...
        {next ? (
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 1, fontWeight: 700 }}>PRÓXIMO</div>
            <div style={{ fontSize: 12, color: "rgba(240,240,240,0.55)" }}>{next.emoji} {next.name}</div>
            <div className="mono" style={{ fontSize: 10, color: "rgba(240,240,240,0.30)" }}>
              −{progress?.needed?.toLocaleString()} XP
            </div>
          </div>
        ) : (
          <div style={{ fontSize: 10, color: "rgba(240,240,240,0.55)", letterSpacing: 1, fontWeight: 700 }}>MÁXIMO 👑</div>
        )}
      </div>
      ...
      {next && progress && (
        <>
          ...
          <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", textAlign: "right", marginTop: 4, fontWeight: 700 }}>
            {progress.pct}%
          </div>
        </>
      )}
      ...
      {expanded && (
        <div style={{ marginTop: 14, borderTop: "1px solid var(--glass-border)", paddingTop: 14, animation: "slideDown 0.2s ease" }}>
          <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 2.5, fontWeight: 700, marginBottom: 10 }}>TODOS LOS RANGOS</div>
          {RANKS.map((r, i) => {
            ...
            return (
              <div key={r.name} style={{
                ...
                {isPast && <span style={{ fontSize: 11, color: "rgba(240,240,240,0.30)" }}>✓</span>}
              </div>
            );
          })}
        </div>
