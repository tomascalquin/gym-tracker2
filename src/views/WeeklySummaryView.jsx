    ...
    <div style={{ maxWidth: 460, margin: "0 auto", fontFamily: "inherit", animation: "fadeIn 0.25s ease" }}>
      ...
      <div style={{ padding: "20px 18px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 3 }}>INTELIGENCIA</div>
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: "var(--text)", letterSpacing: -0.8 }}>Semana actual</h2>
          </div>
        </div>
          ...
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 2, marginBottom: 4 }}>{s.label}</div>
                  <div style={{ fontSize: 22, fontWeight: 300, color: "var(--text)" }}>{s.value}</div>
                  <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", marginTop: 2 }}>{s.sub}</div>
                </div>
              ))}
            </div>
                ...
                {data.prs.map((pr, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 12, color: "#22c55e" }}>
                      {pr.rm}kg <span style={{ fontSize: 10, color: "rgba(240,240,240,0.30)" }}>(antes {pr.prev}kg)</span>
                    </span>
                  </div>
                ))}
              </div>
            ...
            {stagnation.length > 0 && (
              <div style={{
                ...
                <div style={{ fontSize: 11, color: "rgba(240,240,240,0.55)" }}>
            ...
            {(fatigue.fatigueLevel === "high" || fatigue.fatigueLevel === "spike") && (
              <div style={{
                ...
                <div style={{ fontSize: 11, color: "rgba(240,240,240,0.55)" }}>{fatigue.alert}</div>
              </div>
            )}
            ...
            <div style={{
              ...
              {loading && (
                <div style={{ textAlign: "center", padding: "24px 0" }}>
                  <div style={{ fontSize: 11, color: "rgba(240,240,240,0.30)", letterSpacing: 2, animation: "blink 1.2s infinite" }}>
                    ANALIZANDO...
                  </div>
                </div>
              )}
              {!loading && error && (
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(240,240,240,0.30)", marginBottom: 12, lineHeight: 1.6 }}>{error}</div>
              ...
              {!loading && !error && summary && (
                <div>
                  ...
                  <div style={{ fontSize: 13, color: "rgba(240,240,240,0.55)", lineHeight: 1.7 }}>{summary}</div>
                </div>
              )}
              {!loading && !error && !summary && (
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  <div style={{ fontSize: 11, color: "rgba(240,240,240,0.30)", marginBottom: 12, lineHeight: 1.6 }}>
                  ...
                  <button onClick={generateSummary} style={{
                    ...
                    color: "#a78bfa", padding: "10px 24px", borderRadius: 12,
        ...
        {tab === "fatigue" && (
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 2, marginBottom: 12 }}>TONELAJE COMPARATIVO</div>
              ...
              {fatigue.prevWeekVolume > 0 && (
                <div style={{ fontSize: 10, color: "rgba(240,240,240,0.30)", marginTop: 8, textAlign: "right" }}>
            ...
            {fatigue.alert && (
              <div style={{
                ...
                <div style={{ fontSize: 11, color: "rgba(240,240,240,0.55)", lineHeight: 1.7 }}>{fatigue.alert}</div>
              </div>
            )}
            ...
            <div style={{
              ...
              <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", letterSpacing: 2, marginBottom: 12 }}>SERIES/SEMANA POR MÚSCULO</div>
              <div style={{ fontSize: 9, color: "rgba(240,240,240,0.30)", marginBottom: 10, lineHeight: 1.6 }}>
                  ...
                  <div key={muscle} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      ...
                      <span style={{ fontSize: 10, color }}>
                        {sets} <span style={{ color: "rgba(240,240,240,0.30)" }}>/ MEV {range.mev} · MAV {range.mav} · MRV {range.mrv}</span>
                      </span>
                    </div>
                    <div style={{ height: 3, background: "rgba(255,255,255,0.12)", borderRadius: 2, position: "relative", overflow: "visible" }}>
          ...
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            {stagnation.length === 0 ? (
              <div style={{
                ...
                <div style={{ fontSize: 14, color: "rgba(240,240,240,0.55)", marginBottom: 6 }}>Sin plateaus detectados</div>
                <div style={{ fontSize: 11, color: "rgba(240,240,240,0.30)", lineHeight: 1.6 }}>
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: "rgba(240,240,240,0.30)" }}>{s.day}</div>
                      </div>
                      <div style={{
                      ...
                      {s.rmHistory.map((h, j) => {
                        ...
                        return (
                          <div key={j} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                            ...
                            <span style={{ fontSize: 8, color: "rgba(240,240,240,0.30)" }}>{h.rm}</span>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div style={{
                      ...
                      padding: "10px 12px", fontSize: 11, color: "rgba(240,240,240,0.55)", lineHeight: 1.6,
                    }}>
                      💡 {s.suggestion}
                    </div>
                  </div>
